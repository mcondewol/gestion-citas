...
class PacientData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (name,lastname,gender,day_of_birth,address,phone,email,password,sick,medicaments,alergy,created_at) ";
        $pw = password_hash($this->password, PASSWORD_DEFAULT);
		$sql .= "value (\"$this->name\",\"$this->lastname\",\"$this->gender\",\"$this->day_of_birth\",\"$this->address\",\"$this->phone\",\"$this->email\",\"$pw\",\"$this->sick\",\"$this->medicaments\",\"$this->alergy\",$this->created_at)";
		Executor::doit($sql);
	}
